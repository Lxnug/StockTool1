...
def calculate_buy_recommendation(ticker):
    ...
    recommendations = {'current_price': current, 'change': change, 'change_percent': change_percent, 'analysis': []}
...
with tab1:
    ...
    if general_news:
        ...
        for news in general_news[:15]:
            ...
            if sentiment in ['positive', 'veryPositive'] or news.get('priceSensitive'):
...
with tab2:
    ...
    if watchlist_data:
        ...
        df = pd.DataFrame([{
            'Ticker': item['ticker'],
            'Preis ($)': f"{item['quote']['c']:.2f}",
            'Änderung ($)': f"{item['quote']['d']:.2f}",
            'Änderung (%)': f"{item['quote']['dp']:.2f}%",
            'Volumen': f"{item['quote']['v']:,}",
            'High ($)': f"{item['quote']['h']:.2f}",
            'Low ($)': f"{item['quote']['l']:.2f}"
